...
while($data = mysql_fetch_array($result)){
    echo "<center>";
    echo '<img src="'.$data["thumb"].'" class="dis"/></br></br></br><b><h7>'.$data["name"].'</h7></b></br></br></br>';
...
<a class="btn-ghost" href="<?php echo $data["link"]; ?>"  rel="noopener" onclick="javascript:window.open('<?php echo $data['link']; ?>'); return false;" download>DOWNLOAD</a>
  ...
  <?php 
      ...
      $url1 = $name.' : https://perfectmusic.com.ng/file/?article='.$title.$id.'&id='.$id;
      $none = 'https://perfectmusic.com.ng/file/?article='.$title.$id.'&id='.$id;
...
<div class="comm">
<form action="/file/query.php" method="POST">
...
if($count < 1) {
    ...
    $newv = $views + 1;
    $up = "UPDATE files SET downloads='$newv' WHERE id='$id'";
    mysql_query("$up");
    echo "</center>";
